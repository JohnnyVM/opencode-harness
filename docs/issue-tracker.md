...
- **Read current repository**: parse `#<number>`, infer one unambiguous `owner/repository` from the Git remotes, then run `gh issue view <number> --repo <owner>/<repository> --json number,title,body,state,comments,url`.
- **Read named repository**: parse `<owner>/<repository>#<number>`, then run `gh issue view <number> --repo <owner>/<repository> --json number,title,body,state,comments,url`.
- **Read URL**: `gh issue view <url> --json number,title,body,state,comments,url`.
